...
$queryorderlistdetail = "SELECT so.*, i.picture, i.product_name, i.cost FROM sale_order as so INNER JOIN inventory as i ON so.product_id = i.product_id WHERE so.order_id = $order_id";
$rsorderdetail = mysqli_query($conn, $queryorderlistdetail);
$rowdetail = mysqli_fetch_array($rsorderdetail);

$queryorderlisttd = "SELECT DISTINCT time_date FROM sale_order WHERE order_id = $order_id AND so.user_id = $user_id";
